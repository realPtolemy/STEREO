...
// void Tracker::cameraInfoCallback(const CameraInfo& msg) {
//     static bool got_camera_info = false;

//     if (!got_camera_info) {
//         c_.loadCamerInfo(*msg);
//         postCameraLoaded();
...
Tracker::Tracker():
    ...
    // Fixa funktionen loadPinholeCamera
...
//    LOG(INFO) << "Distortion coeffs: " << c_.distortionCoeffs();
    ...
    // Kolla här vad som är bäst får våran kamera!
    auto_trigger_ = false;
        // rpg_common_ros::param<bool>(nhp_, "auto_trigger", false);
...
void Tracker::mapCallback(const pcl::PointCloud<pcl::PointXYZI>::Ptr& msg) {
    ...
    // map_ förväntar sig pcl::PointCloud<pcl::PointXYZ> men msg är pcl::PointCloud<pcl::PointXYZI>
    // Kolla in på detta mer, de kanske är kompatibla med varandra ändå?
    // Ifall vi inte behvöer göra pcl::toPCLPointCloud2 skulle det vara en bra optimisering,
    // då ROS förrut gjorde en liknande sak
    pcl::PCLPointCloud2 pcl_pc;
    pcl::toPCLPointCloud2(*msg, pcl_pc);
