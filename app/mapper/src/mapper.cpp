...
// 	#ifdef USE_UDP
// 	std::thread camera1_thread(local_dsi_udp, "data/camera_0.csv", std::ref(camera1_events));
// 	std::thread camera2_thread(local_dsi_udp, "data/camera_1.csv", std::ref(camera2_events));
// 	#else
// 	threads[0] = std::thread(&Mapper::camera_thread, this, "data/camera_0.csv", std::ref(camera1_events), std::ref(events_left_));
// 	threads[1] = std::thread(&Mapper::camera_thread, this, "data/camera_1.csv", std::ref(camera2_events), std::ref(events_right_));
// 	#endif
...
// */
// void Mapper::camera_thread(const std::string &event_file_path, std::vector<Event> &camera_events, std::deque<std::vector<Event>> &event_queue)
// {
